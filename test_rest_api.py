...
class TestRestAPI(unittest.TestCase):
    ...
    def test_get_all_posts_and_filtering(self):
        ...
        client.post("/posts", json=p1)
        client.post("/posts", json=p2)

        resp_all = client.get("/posts")
        self.assertEqual(resp_all.status_code, 200)
        all_posts = resp_all.json()
        self.assertEqual(len(all_posts), 2)

        # filter by user
        resp_alice = client.get("/posts", params={"user": "alice"})
    ...
    def test_get_posts_by_user_endpoint(self):
        ...
        resp = client.get("/users/alice/posts")
        self.assertEqual(resp.status_code, 200)
        posts = resp.json()
